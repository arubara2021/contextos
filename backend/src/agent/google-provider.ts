...
export class GoogleProvider implements AIProvider {
  ...
  constructor(definition: CloudProviderDefinition) {
    this.name = definition.name || "google";
    this.definition = {
      ...definition,
      baseUrl:
  ...
  private maxOutputTokens(requested: number | undefined): number {
    const fallback = 8192;
    const value =
  ...
  private async chatInternal(
    ...
    const generationConfig: Record<string, unknown> = {
      temperature:
        typeof request.temperature === "number"
          ? request.temperature
          : 0.7,
      maxOutputTokens: this.maxOutputTokens(request.maxTokens),
    };
    ...
    if (json?.promptFeedback?.blockReason) {
      throw new AIError(
        `Google request blocked: ${json.promptFeedback.blockReason}`,
        {
          code: "GOOGLE_BLOCKED",
          provider: this.name,
          retryable: false,
    ...
    if (json?.error?.message && !json?.candidates) {
    ...
    if (candidate?.finishReason === "SAFETY") {
      throw new AIError("Google response blocked by safety filters", {
        code: "GOOGLE_SAFETY",
        provider: this.name,
        retryable: false,
      });
  ...
  private async embedInternal(
    ...
    if (!modelId) {
      throw new AIError(`${this.name} embedding model is not configured`, {
        code: "GOOGLE_EMBEDDING_MODEL_MISSING",
        provider: this.name,
        retryable: false,
      });
    }
    ...
    if (!text) {
      throw new AIError("Cannot embed empty text", {
        code: "GOOGLE_EMBEDDING_EMPTY_TEXT",
        provider: this.name,
        retryable: false,
      });
    }
    ...
    if (json?.error?.message && !json?.embedding) {
      throw new AIError(String(json.error.message), {
    ...
    if (!Array.isArray(vector)) {
      throw new AIError("Google returned an invalid embedding response", {
        code: "GOOGLE_BAD_EMBEDDING_RESPONSE",
        provider: this.name,
        retryable: false,
      });
    }
