...
async function main() {
  ...
  const allKeywords = result.chunks.flatMap((c) => c.keywords);
  ...
  const sections = [...new Set(result.chunks.map((c) => c.section).filter(Boolean))];
  ...
  for (const s of sections) {
    const sectionChunks = result.chunks.filter((c) => c.section === s);
    const sectionTokens = sectionChunks.reduce((sum, c) => sum + c.tokenEstimate, 0);
